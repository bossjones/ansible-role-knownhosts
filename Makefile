...
GITHUB_REPO_NAME                := ansible-role-knownhosts
IMAGE_BASE                      := $(GITHUB_ORG_NAME)/$(GITHUB_REPO_NAME)
IMAGE_TAG                       := $(IMAGE_BASE):$(PR_SHA)

# Build configuration

role_name                 := ansible-role-knownhosts
...
# Docker run command
###############################################################################################################3
DOCKER_RUN=time docker run --rm -e "PLATFORM_NAME=molecule-test-$(PR_SHA)" -e "GIT_SHA=$(PR_SHA)" -e "_DATE=$(_DATE)" -e "TERM=xterm" -w /tmp/ansible-role-knownhosts
...
DOCKER_RUN+= -v "$(CURRENT_DIR):/tmp/ansible-role-knownhosts:rw"
...
.PHONY: run-ubuntu
run-ubuntu:
	ansible-playbook -vvvvv --ask-become-pass -i "localhost," -c local playbook_ubuntu.yml --extra-vars="bossjones__knownhosts__user=$(WHOAMI)"

.PHONY: run-ubuntu-version-manager
run-ubuntu-version-manager:
	ansible-playbook -vvvvv --ask-become-pass -i "localhost," -c local playbook_ubuntu_version_manager.yml --extra-vars="bossjones__knownhosts__user=$(WHOAMI) boss__user=$(WHOAMI) boss__group=$(WHOAMI)"
